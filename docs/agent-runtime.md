...
真实模型层需要端点，平时跑不到 —— 一个从没被执行过的脚本等于死代码。所以它带一个自测模式：

```bash
node evals/agent-bench-llm.js --self-test   # 本地起一个 OpenAI 兼容的假服务，跑同一条代码路径
```

自测会硬断言「请求发得出去、工具调用解析得回来、指标能汇总」，跑不通就红；它输出的是链路结论，
**不代表任何模型能力**，数字不要引用。这条自测第一次跑就抓到一个真问题：假服务说的是 OpenAI 协议，
而客户端在无 apiKey 时按 Ollama 协议发请求，于是解析不到工具调用。已随 `npm test` 一起跑。
...
| P3 密钥可能明文落盘 | **属实** | 已提供「**只本次运行使用，不写入本机**」选项（v1.10.3）：密钥只留在主进程内存，退出/登出即失效；从「保存到本机」切到该模式时会把磁盘上的密文一并摘掉。真实主进程实测 8 条（含数据文件里既无明文也无对应密文）|
