...
// ============================================================
//  旧简历导入器（参考 OpenResume 的「上传 PDF → 自动填表」思路，中文规则版）
...
async function extractPdfText(filePath) {
  ...
  try {
    ...
    throw new Error('无法读取该 PDF：' + (err && err.message ? err.message : '文件损坏或格式不受支持'));
  }
  ...
  try {
    ...
    try { await doc.destroy(); } catch (_) {} // eslint-disable-line no-empty
  }
...
function loadRefData(dataRoot) {
  ...
  try {
    const raw = JSON.parse(fs.readFileSync(path.join(dataRoot, 'cities.json'), 'utf8'));
    raw.forEach((c) => {
      if (!c || !c.name || /^(市辖区|县|省直辖县级行政区划|自治区直辖县级行政区划)$/.test(c.name)) return;
      const n = c.name;
      citySet.add(n);
      if (/市$/.test(n)) citySet.add(n.slice(0, -1));
    });
  } catch (_) {} // eslint-disable-line no-empty
...
function detectSectionKey(line) {
  ...
  for (let i = 0; i < SECTION_DEFS.length; i++) {
    if (SECTION_DEFS[i][1].test(zh) || SECTION_DEFS[i][1].test(s)) return SECTION_DEFS[i][0];
  }
...
function matchPeriod(text) {
  ...
  if (!m) return '';
  const mm = (x) => (x ? String(x).padStart(2, '0') : '');
...
// ---------- 教育解析 ----------
function parseEducation(lines, ref) {
  const entries = [];
  let cur = null;
  ...
  lines.forEach((line) => {
    ...
    if (majorM) {
      ...
      // 「华中科技大学 计算机科学与技术 本科」这类同行混排：去掉已知片段后取剩余中文段
      const rest = line
        .replace(school, '')
...
// ---------- 主入口：结构化解析 ----------
function parseResumeText(text, ref) {
  ...
  // 1) 分节
  const sections = { header: [], education: [], internships: [], projects: [], skills: [], summary: [] };
  let current = 'header';
  ...
  if (!name) {
    // 页眉区前几行里找 2-4 个纯汉字（且不是城市/学校/简历标题词）视为姓名；
    // 按行内空白分词，兼容「李四 13812345678」这类混排行
...
// ---------- 文件入口：按扩展名分流 ----------
async function importFromFile(filePath, dataRoot) {
