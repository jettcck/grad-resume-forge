...
export const KNOWLEDGE: KnowledgeEntry[] = [
  ...
  {
    id: 'app-embedded',
    keywords: ['内置模型', '应用内模型', '免安装', 'webgpu', '下载模型', '不装 ollama', '不想装 ollama', '不装模型', '不用 api', '不想填 api', '免配置', '没有显卡能用 ai 吗'],
    domains: ['all'],
    question: '不想装 Ollama / 不想填 API，还能用 AI 吗',
    answer: '能。「Agent 深度优化 → 配置」里选「应用内模型」：点一次「下载到本机」（约 500MB，走缓存不占安装包），之后 AI 完全在应用内运行，离线可用、什么都不用装。电脑需要有独立显卡或较新的核显（WebGPU），不支持的话这个选项会置灰。',
