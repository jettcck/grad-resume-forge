...
export const KNOWLEDGE: KnowledgeEntry[] = [
  ...
  {
    id: 'tailor',
    keywords: ['一份简历投所有', '针对性', '定制', '不同岗位', '改简历', '通用简历', '多版本', '版本管理', '存一版', '简历版本'],
    domains: ['all'],
    question: '要不要每个岗位改一版简历',
    answer: '投不同方向的岗位（比如既投运营又投销售）至少准备两版，改的是：技能顺序、项目详略、自我介绍方向。\n产品上用「简历版本」来管：信息录入页右侧「简历版本」→「把当前档案存为新版本」，给每版起名（如「字节-后端」「国企-数据」），它会把该版针对的 JD 和当时的体检分 / JD 覆盖率一起记住；下次同一岗位直接点「载入」就回来了，不用重填。\n同一方向内的岗位，再用「JD 匹配」贴一下招聘要求，命中低的技能补进简历再投。',
    action: { label: '去存一版', route: 'profile' }
  },
  // ---------- 通用：求职流程 ----------
  {
    id: 'when-apply',
