...
除 AI 改写外的全部功能**不需要任何模型、密钥或配置**——装上就能用：
...
| Agent 深度优化（AI 改写） | ✅ 三通道任选其一 ⬇ |

AI 三通道，任选其一、随时切换：

1. **应用内模型**（默认，零配置）—— Qwen2.5-0.5B 经 WebGPU 在应用内运行。首次需下载 **约 281MB**，走国内镜像直连（权重 hf-mirror.com / 运行库 gh-proxy.com），**无需科学上网**，之后完全离线可用。需要较新的显卡（WebGPU）。
2. **本地 Ollama** —— 全程不出本机。
...
- **Agent 深度优化**：三通道任选——应用内模型（默认，约 281MB 首次下载后离线跑）/ 本地 Ollama / 云端 API（DeepSeek / Kimi / 通义 / OpenAI，BYOK 密钥只存本机），按 JD 改写条目；每轮产出必须过**确定性校验门**（套话 / 丢数字 / 评分下降一律拒收重生成），档案不会越改越差
...
推荐 —— **应用内模型，零配置**：应用内 ⚙ 配置 → 「应用内模型」（默认已选）→ 点「下载模型到本机」（约 281MB，国内镜像直连、无需科学上网；之后离线运行）。需要 WebGPU。
