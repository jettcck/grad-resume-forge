...
1. **应用内模型**（默认，零配置）—— Qwen2.5-0.5B 经 WebGPU 在应用内运行。首次需下载 **约 270-281MB**，走国内镜像直连（权重 hf-mirror.com / 运行库 gh-proxy.com），**无需科学上网**，按显卡自动选适配档位、网络抖动自动断点续传，之后完全离线可用。需要较新的显卡（WebGPU）。
2. **本地 Ollama** —— 全程不出本机。
