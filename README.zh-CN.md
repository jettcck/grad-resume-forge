...
按 JD 优化有四条通道，任选其一、随时切换（**你选的通道会被记住**：重启后仍然是你选的那条，不会因为探测到可用模型就自动切走）：
...
**490+ 项自动化断言**，push 即跑（CI 门禁）。

| 套件 | 内容 | 命令 |
|---|---|---|
| 引擎自测 | 改写 / 体检 / 匹配 / 词边界回归 | `node test-engine.js` |
| 导入器自测 | 中文简历解析 / PDF 抽取 | `node test-importer.js` |
| Agent 自测 | 校验门 / 重生成 / 流式 / 收工检查 / 云端客户端（mock 全链路） | `node test-agent.js` |
| 渲染层应用改写 | 取消勾选必须保留原文（从 app.js 里切真实函数跑） | `node test-renderer-apply.js` |
| 端到端 | 注册 → 登录 → 存档 → 生成 → 投递 → 快照回炉 → 设置按账号隔离 | `node test-e2e.js` |
| 简历版本 | 存版 / 改名 / 载入 / 上限淘汰 | `node test-versions.js` |
| 数据备份 | 轮转 / 去重 / 保留上限 / 越界文件名拒绝 / 恢复前先快照 | `node test-backup.js` |
| 流式响应 | 无尾换行不丢最后一段 / 多字节跨分片不乱码 | `node test-stream.js` |
| 输入校验 | 体积上限 / 形状校验 / 循环引用 | `node test-validate.js` |
...
| **安全边界（说清楚比藏着好）** | 数据接口的 userId 由主进程按登录会话推导，不接受渲染层传来的 id（防越权）；每个账号的 Agent 配置与云端密钥相互隔离；密钥不回显到界面进程，只回传「是否已保存」；本机没有可用的系统加密（safeStorage）时，界面会**明确提示密钥将明文保存**，请自行判断是否保存；导出 PDF 的离屏窗口关闭脚本执行；各处输入有体积上限。这些都是**本地应用的边界**，不是服务器级安全模型——共用电脑请用系统账户隔离。 |
| **数据文件是单文件 JSON** | 写入是「先写临时文件再改名」的原子写，断电不会留下半截 JSON；文件带 `schemaVersion` 便于日后迁移。没有多进程写队列，但也不需要你操心双开：应用内置**单实例锁**——第二次启动只会把已有窗口带到前台，不会出现两个实例写同一个文件；轮转备份仍是断电与误操作的兜底。 |
